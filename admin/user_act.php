...
if ($filename == "") {
    ...
    if (!in_array($ext, $allowed)) {
        header("location:user.php?alert=gagal-tambah");
    } else {
        move_uploaded_file($_FILES['foto']['tmp_name'], '../assets/dist/img/user/' . $rand . '_' . $filename);
        $file_gambar = $rand . '_' . $filename;
        mysqli_query($koneksi, "insert into user values (NULL,'$nama','$username','$password','$file_gambar','$level')");
        header("location:user.php?alert=berhasil-tambah");
    }
}
